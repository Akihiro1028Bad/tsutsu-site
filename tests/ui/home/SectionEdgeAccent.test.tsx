...
describe("SectionEdgeAccent", () => {
  ...
  it("renders a decorative hairline span on desktop with motion allowed", () => {
    ...
    framerReducedMotion.value = false
    const { container } = render(<SectionEdgeAccent />)
    const accent = container.querySelector(".section__edge-accent") as HTMLElement
  ...
  it("renders with additional className when provided", () => {
    mockMatchMedia({ "(pointer: fine)": true })
    framerReducedMotion.value = false
    const { container } = render(<SectionEdgeAccent className="services__edge-accent" />)
    const accent = container.querySelector(".section__edge-accent") as HTMLElement
    expect(accent).not.toBeNull()
    expect(accent.classList.contains("services__edge-accent")).toBe(true)
  })
  ...
  it("renders nothing on touch devices", () => {
    ...
    framerReducedMotion.value = false
    const { container } = render(<SectionEdgeAccent />)
    expect(container.querySelector(".section__edge-accent")).toBeNull()
  })
