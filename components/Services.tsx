...
const services: Service[] = [
  {
    ...
    backgroundImage: '/services/web-design-bg.jpg', // 背景画像: Webサイトのモックアップやデザインスケッチ
  },
  {
    title: 'Webアプリ / システム開発',
    ...
    backgroundImage: '/services/web-app-bg.jpg', // 背景画像: ダッシュボードやコードエディタのモックアップ
  },
  {
    ...
    gradient: 'from-slate-400 via-slate-500 to-slate-600',
    bgGradient: 'from-slate-50 to-slate-100',
    backgroundImage: '/services/automation-bg.jpg', // 背景画像: ワークフローや自動化を表す抽象的な図
  },
  {
    title: '学習支援・キャリア支援',
    ...
    gradient: 'from-slate-500 via-slate-600 to-slate-700',
    bgGradient: 'from-slate-100 to-slate-200',
    backgroundImage: '/services/learning-bg.jpg', // 背景画像: 成長カーブや学習シーンのイラスト
  },
]
...
function ServiceCard({ service, index }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      {...({} as any)}
    >
      <div className={`flex flex-col md:flex-row items-stretch gap-0 ${isLeft ? '' : 'md:flex-row-reverse'}`}>
        {/* Main Content Block */}
        <motion.div
        ...
        >
          <motion.div
            ...
            className="relative bg-white p-12 md:p-16 md:min-h-[500px] flex flex-col justify-between group border border-slate-200/50 hover:border-slate-300 transition-colors duration-500"
            data-service-card
            {...({} as any)}
          >
            {/* Background Number */}
            <div className="absolute top-8 right-8 text-[10rem] md:text-[16rem] font-light text-slate-50 leading-none select-none pointer-events-none">
              {service.number}
            </div>
            ...
            <div className="relative z-10 space-y-8">
              ...
              {/* Title */}
              <h3 className="text-4xl md:text-5xl lg:text-6xl font-light text-slate-950 tracking-tight leading-[1.1] max-w-lg">
                {service.title}
              </h3>
              ...
              <div className="relative">
                <div className="flex gap-1 border-b border-slate-200" role="tablist">
                  <button
                    ...
                    className={`relative px-6 py-3 text-sm font-semibold uppercase tracking-wider transition-all duration-300 ${
                        ...
                        <ul className="space-y-3">
                          {service.capabilities.map((cap, i) => (
                            <motion.li
                              ...
                              className="text-sm md:text-base text-slate-600 flex items-start gap-3"
                              {...({} as any)}
                            >
        ...
        <motion.div
          ...
          className={`w-full md:w-80 lg:w-96 h-64 md:h-auto bg-gradient-to-br ${service.bgGradient} flex items-center justify-center relative overflow-hidden group-hover:scale-[1.02] transition-transform duration-500 border border-slate-200/50 hover:border-slate-300`}
          {...({} as any)}
        >
          {/* Background Image Overlay (複合アプローチ: CSSフィルター適用) */}
          ...
          {/* Monochrome Overlay (複合アプローチ: モノクロオーバーレイ) */}
          <div 
            className="absolute inset-0 bg-slate-900/12 group-hover:bg-slate-900/15 transition-colors duration-500 z-[1]"
            aria-hidden="true"
          />

          {/* Gradient Overlay (複合アプローチ: グラデーション常時表示) */}
          <motion.div
            animate={{
            ...
            className={`absolute inset-0 bg-gradient-to-br ${service.gradient} opacity-25 group-hover:opacity-30 transition-opacity duration-500 z-[1]`}
            style={{
          ...
          <div className="absolute top-6 right-6 text-7xl md:text-8xl font-light text-slate-200/40 leading-none select-none pointer-events-none z-0">
            {service.number}
          </div>
        </motion.div>
      </div>
