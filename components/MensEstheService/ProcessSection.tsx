  ...
  return (
    <section id="process" aria-labelledby="process-heading" className="relative py-20 md:py-32">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6">
        {isMobile ? (
          <Reveal className="mb-16 text-center" variant="slideUp">
            <h2
              id="process-heading"
        ...
        <ol className="relative mt-16 grid gap-12 md:gap-16">
          {processSteps.map((step, index) => {
            ...
            const ArticleBodyMobile = (
              <>
                <div className="absolute inset-0 bg-gradient-to-br from-gold-50/0 to-gold-100/0 opacity-0 transition-opacity duration-500 group-hover:opacity-100" />

                <div className="relative flex flex-col gap-5 md:flex-row md:items-start md:gap-6">
                  <div className="flex shrink-0 items-center justify-center md:flex-col md:gap-4">
                    <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-gold-500 via-gold-600 to-gold-700 text-xl font-bold text-white shadow-xl ring-4 ring-gold-100 transition-all duration-300 group-hover:ring-gold-200 md:h-20 md:w-20 md:text-2xl">
                      <span aria-label={`ステップ${step.stepNumber}`}>{step.stepNumber}</span>
                      <div className="absolute inset-0 rounded-full bg-gold-400 opacity-0 blur-xl transition-opacity duration-300 group-hover:opacity-50" />
                    </div>
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:gap-4">
                      {Icon ? (
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gold-100 text-gold-700 shadow-md transition-all duration-300 group-hover:bg-gold-200">
                          <Icon aria-hidden="true" className="h-6 w-6" />
                        </div>
                      ) : null}
                      <h3 className="text-xl font-bold tracking-tight text-slate-950 md:text-2xl">
              ...
              <li key={step.id} className="relative">
                ...
                {!isLast && (
                  isMobile ? (
                    <Reveal
                      aria-hidden="true"
                      className="absolute left-8 top-20 h-full w-1 bg-gradient-to-b from-gold-500 via-gold-400 to-gold-300 md:left-10 md:top-24"
                      variant="growY"
                      delayMs={index * 80 + 240}
                      amount={0.2}
                    >
